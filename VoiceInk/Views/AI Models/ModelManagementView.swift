...
enum ModelFilter: String, CaseIterable, Identifiable {
    ...
    case local = "Local"
    // Cloud and Custom hidden for EmberType (100% offline)
    // case cloud = "Cloud"
    // case custom = "Custom"
...
struct ModelManagementView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                // Intel Mac warning removed - EmberType is offline-only, no cloud fallback
                    ...
                    if selectedFilter == .local {
                        HStack(spacing: 8) {
                            ...
                            InfoTip(
                                ...
                                message: "Add a custom fine-tuned whisper model to use with EmberType. Select the downloaded .bin file.",
                                learnMoreURL: "https://embertype.com/docs/custom-local-whisper-models"
                            )
                            .help("Read more about custom local models")
                        }
                    }
                    
                    // Custom model section hidden for EmberType (100% offline)
                }
            }
        }
        .padding()
    ...
    // Intel Mac warning banner removed - EmberType is 100% offline, no cloud option

    private var filteredModels: [any TranscriptionModel] {
        switch selectedFilter {
        case .recommended:
            return whisperState.allAvailableModels.filter {
                let recommendedNames = ["parakeet-tdt-0.6b-v3", "parakeet-tdt-0.6b-v2", "ggml-large-v3-turbo-q5_0", "ggml-base.en"]
                return recommendedNames.contains($0.name)
            }.sorted { model1, model2 in
                let recommendedOrder = ["parakeet-tdt-0.6b-v3", "parakeet-tdt-0.6b-v2", "ggml-large-v3-turbo-q5_0", "ggml-base.en"]
        ...
        // Cloud and Custom cases removed for EmberType (100% offline)
        }
    }
