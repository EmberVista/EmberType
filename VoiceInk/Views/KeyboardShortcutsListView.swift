    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Quick reference for all EmberType shortcuts")
...
private struct KeyBadge: View {
    ...
    var body: some View {
        Text(text)
            .font(DesignTokens.inter(size: 13, weight: .semibold))
...
#Preview {
    KeyboardShortcutsListView()
        .environmentObject(HotkeyManager(whisperState: WhisperState(modelContext: try! ModelContext(ModelContainer(for: Transcription.self)), licenseViewModel: LicenseViewModel())))
}
