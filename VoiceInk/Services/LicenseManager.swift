...
final class LicenseManager {
    ...
    private let logger = Logger(subsystem: "com.embervista.embertype", category: "LicenseManager")

    private let licenseKeyIdentifier = "embertype.license.key"
    private let trialStartDateIdentifier = "embertype.license.trialStartDate"
    private let activationIdIdentifier = "embertype.license.activationId"
    ...
    private func migrateFromUserDefaultsIfNeeded() {
        ...
        // Migrate license key
        if let oldLicenseKey = userDefaults.string(forKey: "EmberTypeLicense"), !oldLicenseKey.isEmpty {
            licenseKey = oldLicenseKey
            userDefaults.removeObject(forKey: "EmberTypeLicense")
        ...
        if let oldActivationId = userDefaults.string(forKey: "EmberTypeActivationId"), !oldActivationId.isEmpty {
            activationId = oldActivationId
            userDefaults.removeObject(forKey: "EmberTypeActivationId")
    ...
    private func getObfuscatedTrialStartDate() -> Date? {
        ...
        let obfuscatedKey = Obfuscator.encode("EmberTypeTrialStartDate", salt: salt)
